...
with aba3:
    ...
    with st.expander("➕ Cadastrar Nova Zona de Estudo", expanded=True):
        ...
        lf_op = z3.number_input("Perda Física Típica (Lf):", value=0.02, format="%.3f")
        # ----------------------------------
        
        pta_op = z1.selectbox("Proteção Choque (PTA):", ["1.0 - Sem proteção", "0.1 - Avisos", "0.01 - Isolação PE", "0.0 - Restrição física"])
        ptu_op = z2.selectbox("Proteção Linha (PTU):", ["1.0 - Sem proteção", "0.1 - Avisos", "0.01 - Isolação", "0.0 - Restrição física"], index=2)
        ...
        if st.button("Salvar Zona na Lista"):
            ...
            # Repassando LF para a classe da zona
            nova_zona = ZonaEstudo(
                id_zona=id_z, n_z=nz_z, n_t=nt_z, t_z=tz_z,
                r_t=rt, r_f=rf, r_p=rp, h_z=hz, l_f=lf_op, p_ta=pta, p_tu=ptu,
