...
def verificar_login():
    ...
    if not st.session_state.autenticado:
        ...
        st.markdown("<h4 style='text-align: center; color: gray;'>Faça login para acessar o sistema de cálculos</h4>",
                    unsafe_allow_html=True)

        # Cria uma caixa centralizada para o formulário
                ...
                if botao_login:
                    ...
                    try:
                        ...
                        if usuario in senhas_salvas and senhas_salvas[usuario] == senha:
                            ...
                            st.rerun()  # Recarrega a página autenticada
                        else:
                            st.error("Usuário ou senha incorretos.")
                    except KeyError:
                        st.error("Erro no servidor: 'Secrets' não configurado corretamente.")

        # Interrompe a execução do restante da página se não estiver logado
        st.stop()


# Chama a função de bloqueio logo no início
verificar_login()

# Botão de Logout no canto superior (só aparece se estiver logado)
...
# Estado da Sessão para guardar as listas
...
with aba1:
    ...
    with col1:
        ...
        l_est = st.number_input("Comprimento L (m):", value=40.0)
        w_est = st.number_input("Largura W (m):", value=30.0)
        h_min = st.number_input("Altura Corpo Principal H_min (m):", value=15.0)
        h_max = st.number_input("Altura Saliência/Antena H_max (m):", value=20.0)
        cd_op = st.selectbox("Fator de Localização (CD):",
                             ["1.0 - Isolada", "0.5 - Cercada mesma altura", "0.25 - Cercada objetos altos",
                              "2.0 - Topo de colina/monte"])

    with col2:
        st.subheader("Estrutura Adjacente (Vizinhança Conectada)")
        tem_adj = st.checkbox("Considerar Estrutura Adjacente Conectada por Linha")
        lj = st.number_input("Comprimento Adjacente LJ (m):", value=10.0, disabled=not tem_adj)
        wj = st.number_input("Largura Adjacente WJ (m):", value=10.0, disabled=not tem_adj)
        hj = st.number_input("Altura Adjacente HJ (m):", value=5.0, disabled=not tem_adj)
        cdj_op = st.selectbox("Fator CDJ:", ["1.0 - Isolada", "0.5 - Mesma altura", "0.25 - Mais baixa"],
                              disabled=not tem_adj)
...
with aba2:
    ...
    with st.expander("➕ Cadastrar Nova Linha Elétrica", expanded=True):
        ...
        ll_lin = c3.number_input("Comprimento LL (m):", value=1000.0)

        ci_op = c1.selectbox("Fator Instalação (CI):", ["1.0 - Aérea", "0.5 - Enterrada", "0.01 - Enterrada em Malha"])
        ce_op = c2.selectbox("Fator Ambiental (CE):",
                             ["1.0 - Rural", "0.5 - Suburbano", "0.1 - Urbano", "0.01 - Urbano (>20m)"])
        ct_op = c3.selectbox("Fator do Tipo (CT):",
                             ["1.0 - Linha BT / Sinal sem Trafo", "0.2 - Linha AT com Trafo Isolador"])

        blindada = c1.checkbox("Linha Blindada")
        rs_lin = c2.number_input("Resistência Blindagem (Ohm/km):", value=1.0)
        uw_lin = c3.number_input("Tensão Suportável UW (kV):", value=1.5)
...
with aba3:
    ...
    with st.expander("➕ Cadastrar Nova Zona de Estudo", expanded=True):
        ...
        id_z = z1.text_input("ID da Zona:", "Zona_1_Escritorios")
        nz_z = z2.number_input("Pessoas na Zona (nz):", value=50.0)
        nt_z = z3.number_input("Pessoas na Estrutura Total (nt):", value=200.0)
        tz_z = z1.number_input("Tempo Exposição (tz h/ano):", value=2000.0)

        rt_op = z2.selectbox("Fator do Piso (rt):",
                             ["1e-5 - Asfalto/Madeira", "1e-4 - Carpete", "1e-3 - Mármore", "1e-2 - Terra/Concreto"])
        rf_op = z3.selectbox("Risco de Incêndio (rf):",
                             ["1.0 - Explosão", "0.1 - Alto", "0.01 - Normal", "0.001 - Baixo", "0.0 - Sem Risco"])
        pta_op = z1.selectbox("Proteção Choque Estrutura (PTA):",
                              ["1.0 - Sem proteção", "0.1 - Avisos", "0.01 - Isolação PE/Malha",
                               "0.0 - Restrição física"])
        ptu_op = z2.selectbox("Proteção Choque Linha (PTU):",
                              ["1.0 - Sem proteção", "0.1 - Avisos", "0.01 - Isolação", "0.0 - Restrição física"])

        st.markdown("**Custos Financeiros na Zona (R$)**")
        c_a = z1.number_input("Animais (ca):", value=0.0)
        c_b = z2.number_input("Edificação (cb):", value=5000000.0)
        c_c = z3.number_input("Conteúdo (cc):", value=2000000.0)
        c_s = z1.number_input("Sistemas (cs):", value=1000000.0)
        c_z = z2.number_input("Patrimônio Cultural (cz):", value=0.0)
        c_t = z3.number_input("Total da Estrutura (ct):", value=8000000.0)
        ...
        if st.button("Salvar Zona na Lista"):
            ...
            nova_zona = ZonaEstudo(
                ...
                r_t=rt, r_f=rf, p_ta=pta, p_tu=ptu,
    ...
    if st.session_state.zonas:
        ...
        df_z = pd.DataFrame([{
            "ID": z.id_zona, "Pessoas (nz)": z.n_z, "Horas (tz)": z.t_z, "rf": z.r_f,
...
with aba4:
    ...
    peb_op = st.selectbox("Equipotencialização de Entrada (PEB):",
                          ["1.0 - Sem DPS Classe I", "0.05 - DPS Nível III-IV", "0.02 - DPS Nível II",
                           "0.01 - DPS Nível I"])
...
with aba5:
    ...
    if st.button("🚀 Executar Análise de Risco Completa", type="primary"):
        if not st.session_state.zonas or not st.session_state.linhas:
            ...
            est = Estrutura(l=l_est, w=w_est, h_min=h_min, h_max=h_max, c_d=cd, tem_adjacente=tem_adj, l_j=lj, w_j=wj,
                            h_j=hj, c_dj=cdj)
            z1 = st.session_state.zonas[0]
            probs = Probabilidades(nivel_spda=spda_op, nivel_dps=dps_op, p_ta=z1.P_TA, p_tu=z1.P_TU, p_eb=peb,
                                   fatores_k=z1.fatores_k)

            analisador = AnalisadorRisco(n_g=ng, estrutura=est, zonas=st.session_state.zonas,
                                         linhas=st.session_state.linhas, probabilidades=probs)
            ...
            c1.metric("Risco R1 (Vida)", f"{r1:.2e}", delta="Aprovado" if r1 <= 1e-5 else "Reprovado",
                      delta_color="normal" if r1 <= 1e-5 else "inverse")
            c2.metric("Risco R3 (Patrimônio)", f"{r3:.2e}", delta="Aprovado" if r3 <= 1e-4 else "Reprovado",
                      delta_color="normal" if r3 <= 1e-4 else "inverse")
            c3.metric("Frequência F (Danos)", f"{f_total:.2f}/ano", delta="Seguro" if f_total <= 1.0 else "Inseguro",
                      delta_color="normal" if f_total <= 1.0 else "inverse")
            c4.metric("Custo Provável (R4)", f"R$ {custo_perda:,.2f}")

            try:
                resultados_dict = {'r1': r1, 'r3': r3, 'f': f_total, 'r4': r4, 'custo_perda': custo_perda}
                pdf_data = gerar_pdf_laudo(cidade, ng, est, st.session_state.zonas, st.session_state.linhas,
                                           resultados_dict)
                ...
                st.error("Módulo de PDF não encontrado ou incompleto. Finalize a configuração.")